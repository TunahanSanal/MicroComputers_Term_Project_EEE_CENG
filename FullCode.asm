;******************************************************************************
; PROJECT: Air Conditioner - V6 FINAL (No Double Press, Long Success, Smooth)
...
ENTER_EDIT_MODE:
    ...
    MOVWF INPUT_INT_VAL
    MOVWF DIGIT_1_VAL
    CALL WAIT_KEY_RELEASE ; <-- 22 yazmas?n diye burada s?k? bekliyoruz
...
WAIT_HASH:
    ...
    ; --- DÜZELTME: S HARF? DAHA UZUN KALSIN ---
    CALL DELAY_1SEC
    CALL DELAY_1SEC
    CALL DELAY_1SEC
...
;------------------------------------------------------------------------------
; KEYPAD & UART FONKS?YONLARI (GÜÇLEND?R?LD?!)
;------------------------------------------------------------------------------
GET_VALID_KEY:
GK_WAIT_NO_KEY:
    CALL SCAN_KEYPAD_RAW
...
; --- DÜZELTME: Ç?FT YAZMAYI ENGELLEYEN FONKS?YON ---
WAIT_KEY_RELEASE:
    CALL DELAY_50MS
WR_LOOP:
    CALL SCAN_KEYPAD_RAW
    XORLW 0xFF
    BTFSS STATUS, Z ; Tu? yok mu (0xFF mi?)
    GOTO WR_LOOP    ; Tu? var, beklemeye devam et.
    
    ; Tu? b?rak?ld? gibi görünüyor, emin olmak için biraz daha bekle ve tekrar bak
    CALL DELAY_50MS
    CALL SCAN_KEYPAD_RAW
    XORLW 0xFF
    BTFSS STATUS, Z ; Hala b?rak?lm?? m??
    GOTO WR_LOOP    ; Hay?r, gürültüymü?, beklemeye devam et.
    
    RETURN ; Evet, kesinlikle b?rak?ld?.
